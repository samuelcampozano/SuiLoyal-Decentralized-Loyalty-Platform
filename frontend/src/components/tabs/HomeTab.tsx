...
export const HomeTab: FC<HomeTabProps> = ({
  ...
  console.log('🏠 HomeTab - Received transactions:', transactions);
  console.log('🏠 HomeTab - Total transactions:', transactions.length);
  console.log('🏠 HomeTab - Transaction types:', transactions.map(t => ({ id: t.id, type: t.type, amount: t.amount })));
        ...
        <div className="space-y-3">
          {transactions.length === 0 ? (
            <div className="text-center py-8 text-gray-500">
              ...
              <p className="text-sm">
                {loyaltyAccount 
                  ? 'Start earning points from merchants or redeem rewards to see your activity here!' 
                  : 'Create a loyalty account to start tracking your activity'}
              </p>
            </div>
          ) : (
              ...
              <div key={tx.id} className="flex justify-between items-center p-3 bg-gray-50 rounded-lg">
                <div>
                  <span className={`inline-block px-2 py-1 rounded text-xs font-semibold ${
                    ...
                    tx.type === 'redeemed' ? 'bg-red-100 text-red-800' : 
                    'bg-gray-100 text-gray-800'
                  }`}>
                    {tx.type === 'earned' ? 'Earned' : tx.type === 'redeemed' ? 'Redeemed' : 'Activity'}
                  </span>
                  <span className="ml-2 text-sm">{tx.merchant}</span>
                  {tx.reward && <span className="ml-2 text-sm text-gray-500">• {tx.reward}</span>}
                </div>
                <div className="text-right">
                  <div className={`font-bold ${tx.amount > 0 ? 'text-green-600' : tx.amount < 0 ? 'text-red-600' : 'text-gray-600'}`}>
                    {tx.amount > 0 ? '+' : tx.amount < 0 ? '-' : ''}{tx.amount === 0 ? 'Activity' : `${Math.abs(tx.amount)} pts`}
                  </div>
